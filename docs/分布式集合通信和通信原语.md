...
**集合通信（Collective Communication）**是指一个进程组（Process Group）内的所有进程都参与的全局通信操作。基础的通信操作包括**发送（Send）**、**接收（Receive）**、**组内进程栅障同步（Barrier）**以及**事件同步（Signal + Wait）**。
...
集合通信的难点在于优化**通信效率**以及选择**最适用于**特定网络硬件拓扑结构的通信算法。

## 通信原语详解

本节以一台配置了四张训练加速卡（XPU，泛指 GPU、NPU、TPU 等）的服务器为例，如下图所示。服务器内的四张加速卡通常是**全连接**的，其物理连接方式可以是专用高速互连（如 **NVLink**），也可以是支持缓存一致性的扩展互连（如 **CXL**）或通用总线/网络标准（如 **PCIe**、**InfiniBand** 等）。我们将基于这一物理拓扑结构来描述集合通信中常用的通信原语。
...
如下图所示，当主节点 GPU 0 执行 Broadcast 时，数据从 GPU 0 被广播至所有其他 GPU 节点（1、2、3）。
...
如下图所示，主节点 0 将数据 $D = [A, B, C, D]$ 切分为四份（不同颜色方块代表不同的数据分片），分别分发给节点 0、1、2、3（根节点自己也保留一份）。
...
- 将分布式数据汇总到所有节点，适用于需要**全局数据**同步的场景。
...
Reduce-Scatter 等价于先在所有节点上执行一次 Reduce 运算，再将规约结果按节点数进行切分，最后进行一次 Scatter 操作。它是 **All-Gather** 的**反向操作**。

如下图所示，假设执行 **SUM** 规约。每个 GPU 持有向量 $A, B, C, D$，其中 GPU 0 持有 $A = [A_0, A_1, A_2, A_3]$，GPU 1 持有 $B = [B_0, B_1, B_2, B_3]$，以此类推。先按元素求和，得到完整结果 $[\sum A_i, \sum B_i, \sum C_i, \sum D_i]$；然后切片发散：GPU 0 获得 $A_0+B_0+C_0+D_0$，GPU 1 获得 $A_1+B_1+C_1+D_1$，GPU 2 获得 $A_2+B_2+C_2+D_2$，GPU 3 获得 $A_3+B_3+C_3+D_3$。

```mermaid
graph TD
    subgraph Reduce Phase
        A["GPU 0<br/>A = [A₀,A₁,A₂,A₃]"] --> R["[A₀+B₀+C₀+D₀,<br/>A₁+B₁+C₁+D₁,<br/>A₂+B₂+C₂+D₂,<br/>A₃+B₃+C₃+D₃]"]
...
**核心功能**：先对多节点数据进行规约（如求和），再将结果分散到各节点，使每个节点仅保留**部分**规约结果。
...
- **All-Gather** 中，每个节点持有**相同**的一份完整数据副本。
- **All-To-All** 中，每个节点收到的数据分片**来源不同**，相当于对数据按列重新排列（矩阵转置）。

**核心功能**：每个节点将自己的数据按切片发送给所有节点，同时接收所有节点的对应切片，实现数据的**全局重排（转置）**。
...
| :-------- | :--------- | :--------- | :--------- | :--------- | :--------------------------- |
| **GPU 0** | $A_{0}$    | $B_{0}$    | $C_{0}$    | $D_{0}$    | $A_0, B_0, C_0, D_0$        |
| **GPU 1** | $A_{1}$    | $B_{1}$    | $C_{1}$    | $D_{1}$    | $A_1, B_1, C_1, D_1$        |
| **GPU 2** | $A_{2}$    | $B_{2}$    | $C_{2}$    | $D_{2}$    | $A_2, B_2, C_2, D_2$        |
| **GPU 3** | $A_{3}$    | $B_{3}$    | $C_{3}$    | $D_{3}$    | $A_3, B_3, C_3, D_3$        |

可以观察到，输入按**行分块**，输出按**列分块**，这正是矩阵转置的语义。
...
graph LR
    ...
    subgraph All-To-All交换
        A0 --> A1[GPU 0<br>A₀,B₀,C₀,D₀]
        B0 --> B1[GPU 1<br>A₁,B₁,C₁,D₁]
        C0 --> C1[GPU 2<br>A₂,B₂,C₂,D₂]
        D0 --> D1[GPU 3<br>A₃,B₃,C₃,D₃]
    end
...
**Signal 与 Wait** 属于**记录型信号量机制**，用于实现进程间的**事件同步**。常见的用法是：发送方通过 `record_stream()` 或显式事件（Event）标记数据就绪，接收方通过等待该事件确保数据已经写入完成，从而保证数据传输的**有序性和完整性**。在 NCCL/HCCL 等集合通信库中，具体语义通常由通信后端封装，用户一般通过 `wait`/`wait_for` 等接口进行同步。
...
各家 AI 芯片/加速卡厂商都会提供私有的网络通信库（如 `xxx-AWARE` 或 `xCCL`）来实现对底层通信硬件的屏蔽与抽象。通信库后端层需要根据厂商 SDK 接口，结合实际网络拓扑结构，实现对应的**最优通信拓扑算法**。
