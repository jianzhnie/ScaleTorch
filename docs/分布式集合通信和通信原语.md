...
如下图所示，假设执行 SUM 规约：先对 GPU 0-3 上的数据进行规约，得到完整结果 ${\sum_{i=0}^{3}A_i | \sum_{i=0}^{3}B_i | \sum_{i=0}^{3}C_i| \sum_{i=0}^{3}D_i}$。然后，将这个结果切片（Scatter）分发给所有 GPU 卡，如 GPU 0 获得 $Sum_0=(A_0+ B_0+C_0+D_0)$ 分片，GPU 1 获得 $sum_1=(A_1+B_1+C_1+D_1)$ 分片，GPU 2 获得 $sum_2=(A_2+B_2+C_2+D_2)$ 分片，GPU 3 获得 $sum_3=(A_3+B_3+C_3+D_3)$ 分片。

```mermaid
graph TD
    subgraph Reduce Phase
        A[GPU 0 <br> A = A₀,A₁,A₂,A₃] --> R(A₀+B₀+C₀+D₀, <br> ... <br> A₃+B₃+C₃+D₃)
