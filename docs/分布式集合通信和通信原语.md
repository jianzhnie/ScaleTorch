...
如下图所示，当主节点 GPU 0 执行 Broadcast 时，数据从主节点 0 被广播至所有GPU 节点（1、2、3）。
...
如下图所示，假设执行 SUM 规约。每个 GPU 持有向量 $A, B, C, D$，其中 GPU 0 持有 $A = [A_0, A_1, A_2, A_3]$，GPU 1 持有 $B = [B_0, B_1, B_2, B_3]$，以此类推。先按位规约求和，得到完整结果 $[\sum A_i, \sum B_i, \sum C_i, \sum D_i]$。然后切片发散：GPU 0 获得 $A_0+B_0+C_0+D_0$，GPU 1 获得 $A_1+B_1+C_1+D_1$，GPU 2 获得 $A_2+B_2+C_2+D_2$，GPU 3 获得 $A_3+B_3+C_3+D_3$。
