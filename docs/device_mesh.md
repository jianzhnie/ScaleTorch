...
```mermaid
graph TB
    hw["物理设备层<br/>rank 0 ~ rank 7"]
    pg["ProcessGroup<br/>NCCL / HCCL 通信域"]
    dm["DeviceMesh<br/>多维 rank 布局 (2,4) / (2,2,2)"]
    dt["DTensor<br/>Shard / Replicate / Partial"]

    subgraph strategies["并行策略"]
        tp["Tensor Parallel"]
