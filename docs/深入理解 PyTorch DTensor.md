...
```mermaid
graph TB
    dm["DeviceMesh<br/>设备 N 维拓扑<br/>┬ shape=(2,4)<br/>├ dims: dp, tp<br/>└ 内部: rank 矩阵 + ProcessGroup"]

    pl["Placement<br/>每维的分布策略<br/>┬ Shard(dim)<br/>├ Replicate()<br/>└ Partial(op)"]

    spec["DTensorSpec<br/>分布身份证<br/>┬ mesh: DeviceMesh<br/>├ placements: List[Placement]<br/>└ tensor_meta: shape/stride/dtype"]

    dt["DTensor<br/>用户可见的分布式张量<br/>┬ to_local() → torch.Tensor<br/>├ redistribute() → DTensor<br/>└ __torch_dispatch__ → 自动通信"]
...
stateDiagram-v2
    ...
    Replicate --> Shard : local_chunk (no comm)
...
distribute_tensor(
    tensor,              # 全局逻辑张量
    device_mesh=None,    # DeviceMesh，默认用当前上下文
    placements=None,     # List[Placement]，默认全 Replicate
    *, src_data_rank=0,  # ⭐ 新增: 源数据所在 rank，None 表示用本地数据直接切分
)
```
...
dtensor_shard = distribute_tensor(
    tensor, device_mesh=mesh, placements=[Shard(0)]
)
# 默认从 src_data_rank=0 取数据，scatter 到各 rank
```

> **`src_data_rank`**（PyTorch 2.12+）：控制从哪个 rank 读取源数据。设为 `None` 时直接使用各 rank 上的本地张量做切分，不触发广播——适用于每个 rank 已持有正确分片的场景。

**方式二：`DTensor.from_local()`（零拷贝，用户保证数据正确性）**

从每个 rank 上已有的本地张量构建 DTensor，**不触发任何数据搬运**：

```python
# 完整签名 (PyTorch 2.12+)
DTensor.from_local(
    local_tensor,           # 本地 torch.Tensor
    device_mesh=None,       # DeviceMesh
    placements=None,        # List[Placement]
    *, run_check=False,     # ⭐ 跨 rank 一致性校验（含通信）
    shape=None,             # ⭐ 全局 shape（各 rank 形状不同时必填）
    stride=None,            # ⭐ 全局 stride
    grad_placements=None,   # ⭐ 反向梯度放置策略
)
```

**`grad_placements`**（PyTorch 2.12+）控制反向传播时输出梯度的放置方式。未指定时默认规则为：
- `Shard → Shard`、`Replicate → Replicate`、`Partial → Replicate`

即 `Partial` 永远不会作为梯度输出——grad 总是被 all-reduce 为 `Replicate`。

```python
from torch.distributed.tensor import DTensor

# 从各 rank 已加载的分片构建（零通信）
local_shard = torch.load(f"model_shard_{rank}.pt")
...
# 完整签名 (PyTorch 2.12+)
dt.redistribute(
    device_mesh=None,     # 目标 DeviceMesh（默认不变）
    placements=None,      # 目标 placements
    *, async_op=False,    # ⭐ 异步操作
    forward_dtype=None,   # ⭐ 前向通信前转换 dtype
    backward_dtype=None,  # ⭐ 反向通信前转换梯度 dtype
)
```

```python
local = dt.to_local()            # 获取本地张量（零开销），支持 grad_placements
full  = dt.full_tensor()         # 全局 all-gather，等价于 redistribute([Replicate]*N)
dt2   = dt.redistribute(mesh, [Replicate()])  # 切换 Placement，自动插入通信
```

**Placement 转换的底层通信**（与 §2.2.1 状态图一致）：

| From → To | 通信操作 |
|---|---|
| `Shard(dim) → Replicate()` | `all_gather` |
| `Shard(src) → Shard(dst)` | `all_to_all` |
| `Replicate() → Shard(dim)` | `torch.chunk`（本地，无通信） |
| `Partial() → Replicate()` | `all_reduce` |
| `Partial() → Shard(dim)` | `reduce_scatter` |

> ⚠️ `full_tensor()` 会触发全量通信，仅用于调试、检查点保存或数值验证等**非热路径**场景。
>
> **2025 梯度改进**：反向传播中 `Partial → Replicate` 是保证行为——DTensor 不会让梯度停留在 `Partial` 状态，避免用户意外拿到未规约的梯度值。
...
```mermaid
graph TB
    user["用户代码: y = matmul(x, w)"]

    subgraph step["DTensor 五步调度"]
        s1["① 拦截 Interception<br/>__torch_dispatch__ 捕获调用<br/>优先级高于 CUDA/HCCL backend"]
        s2["② 拆包 Unpacking<br/>DTensor → local_tensor + DTensorSpec<br/>组装 OpInfo 结构"]
        s3["③ Sharding 推导 Propagation<br/>判断输出 Placement<br/>决定是否需要重分布"]
        s4["④ 通信 + 本地计算<br/>需重分布 → all_gather/reduce_scatter<br/>本地 → aten.mm / aten.add / ..."]
        s5["⑤ 重包装 Repacking<br/>torch.Tensor + spec → DTensor<br/>后续算子继续 DTensor 调度"]
        s1 --> s2 --> s3 --> s4 --> s5
    end

    user --> s1
    s5 --> result["用户看到: y 仍是 DTensor<br/>无需关心通信细节"]
```
...
**2025 梯度改进**（PyTorch 2.12+）：

- **`Partial → Replicate` 保证**：反向传播中 `Partial` 梯度永远不会作为输出——DTensor 自动将其 all-reduce 为 `Replicate`。这避免了用户意外拿到未规约的梯度值。
- **`grad_placements` 参数**（`DTensor.from_local` / `to_local`）：允许用户显式控制反向传播中梯度的放置策略。
- **正向→反向 Placements 映射**：`Shard → Shard`、`Replicate → Replicate`、`Partial → Replicate`（保证行为）。
- **移除 `is_backward`**：内部 `redistribute_local_tensor` 不再通过 flag 区分前后向，反向行为由 `grad_placements` 机制统一管理。
...
反向：输入梯度 `matmul(input_grad: Shard[0], param: Replicate) → input_grad: Shard[0]`（无通信）；参数梯度 `matmul(input: Shard[0], output_grad: Shard[0]) → param_grad: Partial` → 自动触发 `all_reduce` → `Replicate`（2025 保证行为）。
...
graph TB
    subgraph user["用户层 (User Layer)"]
        code["模型代码: model.forward(x)<br/>并行配置: parallelize_module(...)"]
    end

    subgraph dtensor["DTensor 层 (Abstraction Layer)"]
        dt["DTensor = local_tensor + DTensorSpec"]
        spec["DTensorSpec: mesh + placements + tensor_meta"]
        ops["OpDispatcher: __torch_dispatch__<br/>拦截 → 拆包 → 推导 → 通信+计算 → 重包装"]
    end

    subgraph strategies["并行策略 (Strategy Layer)"]
        dp["DDP<br/>Replicate params<br/>Shard(0) input"]
        tp["TP<br/>ColwiseParallel<br/>RowwiseParallel"]
        sp["SP<br/>Shard(0) seq<br/>input+output"]
        fsdp["FSDP<br/>Shard(0) params<br/>all-gather → drop"]
        hsdp["FSDP+DP<br/>2D Mesh<br/>Replicate+Shard"]
    end

    subgraph comm["通信层 (Communication Layer)"]
        hccl["HCCL / NCCL<br/>ProcessGroup<br/>all_reduce / all_gather<br/>reduce_scatter / all_to_all"]
    end
...
**官方 API 文档**：
- **[`torch.distributed.tensor`](https://docs.pytorch.org/docs/main/distributed.tensor.html)** — DTensor API 参考（含 2025 新增参数）
- **[Getting Started with DeviceMesh](https://docs.pytorch.org/tutorials/recipes/distributed_device_mesh.html)** — DeviceMesh 官方教程
...
**2025 关键 PR**：
- **[#160266](https://github.com/pytorch/pytorch/pull/160266)** — `redistribute` 支持 device ordering
- **[#170527](https://github.com/pytorch/pytorch/pull/170527)** — `grad_placements` 用户控制梯度放置
- **[#170355](https://github.com/pytorch/pytorch/pull/170355)** — 移除 `is_backward`，统一前后向梯度管理
