...
class TestProcessGroupManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patchers = [
            patch(
                "scaletorch.parallel.process_group.is_distributed", return_value=True
            ),
            patch("scaletorch.parallel.process_group.get_rank", return_value=0),
            patch("scaletorch.parallel.process_group.get_world_size", return_value=8),
            patch(
                "scaletorch.parallel.process_group.new_group", return_value=MagicMock()
            ),
        ]
        (
            self.mock_is_distributed,
            self.mock_get_rank,
            self.mock_get_world_size,
            self.mock_new_group,
        ) = [p.start() for p in patchers]
        self._patchers = patchers

        # Clear global process group manager proxy instance
    ...
    def test_init_valid_configuration(self):
        ...
        with patch.object(ProcessGroupManager, "_initialize_group_properties"):
            process_group = ProcessGroupManager(
                tp_size=2, cp_size=2, pp_size=2, dp_size=1
            )
    ...
    def test_init_distributed_not_initialized(self):
        ...
        self.assertIn(
            "Distributed training must be initialized", str(context.exception)
        )
    ...
    def test_init_invalid_parallelism_sizes(self):
        ...
        for tp_size, cp_size, pp_size, dp_size, expected_msg in test_cases:
            with self.assertRaises(ValueError) as context:
                ProcessGroupManager(
                    tp_size=tp_size, cp_size=cp_size, pp_size=pp_size, dp_size=dp_size
                )
    ...
    def test_setup_process_group_manager(self):
        ...
        with patch.object(ProcessGroupManager, "_initialize_group_properties"):
            process_group = setup_process_group_manager(
                tp_size=2, cp_size=2, pp_size=2, dp_size=1
            )
            ...
            self.assertIs(pgm.process_group_manager._instance, process_group)
    ...
    def test_get_process_group_manager(self):
        ...
        with patch.object(ProcessGroupManager, "_initialize_group_properties"):
            setup_process_group_manager(tp_size=2, cp_size=2, pp_size=2, dp_size=1)
            manager = get_process_group_manager()
