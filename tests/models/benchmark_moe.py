...
def run_benchmarks() -> None:
    ...
    configs = [
        {"name": "Standard GPT", "config": {**base_config, "use_moe": False}},
        {
            "name": "MoE (2 experts)",
    ...
    print("=" * 50)
    print(
        f"{'Configuration':<25} {'Avg Time (s)':<15} {'Throughput (tok/s)':<20} {'Params (M)':<15}"
    )
    print("-" * 75)
    ...
    for config_info in configs:
        ...
        try:
            ...
            print(
                f"{name:<25} {result['avg_time']:<15.4f} {result['avg_throughput']:<20.2f} "
                f"{result['total_params'] / 1e6:<15.2f}"
            )
        except Exception as e:
            print(f"{name:<25} Failed with error: {e!s}")
    ...
    if results:
        ...
        print(
            f"Smallest model: {smallest[0]} ({smallest[1]['total_params'] / 1e6:.2f}M parameters)"
        )
