...
for model_path, desc in MODELS:
    ...
    try:
        ...
        if getattr(config, 'tie_word_embeddings', False):
            assert model.final_proj.weight.data_ptr() == model.embedding.weight.data_ptr(), \
                "tie_word_embeddings: weights should share memory"
            print(f"  Tie check: PASS (shared memory)")

        x = torch.randint(0, config.vocab_size, (1, 32), device=device)
        with torch.no_grad(), torch.amp.autocast(device_type="npu", dtype=torch.bfloat16):
            out = model(x)
