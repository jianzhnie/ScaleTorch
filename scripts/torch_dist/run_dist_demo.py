...
def init_dist_process(use_cpu: bool = False, timeout: int = 120) -> None:
    ...
    try:
        ...
        rank_print(
            rank, f"Initialized distributed process group with backend: {backend}"
        )
    except Exception as e:
        rank_print(rank, f"Failed to initialize distributed process group: {e}")
        raise
...
def test_gather(rank: int, world_size: int, device: torch.device) -> None:
    ...
    try:
        # Create local data
        local_data = torch.tensor([rank, rank * 2], dtype=torch.float32, device=device)
...
def test_scatter(rank: int, world_size: int, device: torch.device) -> None:
    ...
    try:
        ...
        if rank == 0:
            ...
            full_data = torch.arange(world_size * 4, dtype=torch.int64, device=device)
            # Use split instead of chunk for more predictable behavior
            scatter_list = list(torch.split(full_data, 4))
            rank_print(
                rank, f"Rank 0 scattering data: {[t.tolist() for t in scatter_list]}"
            )
...
def test_reduce(rank: int, world_size: int, device: torch.device) -> None:
    ...
    try:
        # Create local data
        tensor = torch.tensor(
            [rank, rank + 1, rank + 2], dtype=torch.float32, device=device
        )
        ...
        if rank == 0:
            rank_print(
                rank, f"After reduce - Rank {rank} (destination) has data: {tensor}"
            )
    except Exception as e:
...
def test_all_gather(rank: int, world_size: int, device: torch.device) -> None:
    ...
    try:
        ...
        local_data = torch.tensor(
            [rank, rank * 2, rank * 3], dtype=torch.float32, device=device
        )
        rank_print(rank, f"Local data - Rank {rank} has data: {local_data}")
...
def test_reduce_scatter(rank: int, world_size: int, device: torch.device) -> None:
...
def test_all_to_all(rank: int, world_size: int, device: torch.device) -> None:
    ...
    try:
        ...
        for i in range(world_size):
            tensor = torch.tensor(
                [rank * world_size + i], dtype=torch.float32, device=device
            )
            input_list.append(tensor)
...
def test_object_broadcast(rank: int, world_size: int, device: torch.device) -> None:
    ...
    try:
        ...
        rank_print(rank, f"Before object broadcast - Rank {rank} has data: {obj_list}")
        dist.broadcast_object_list(obj_list, src=0)
        rank_print(rank, f"After object broadcast - Rank {rank} has data: {obj_list}")
    except Exception as e:
...
def run_all_tests(use_cpu: bool = False) -> None:
    ...
    rank_print(rank, f"Starting distributed tests on Rank {rank}/{world_size - 1}")
