...
def run_config(cfg, port):
    ...
    for line in output.split("\n"):
        ...
        if m:
            ...
            mfu = float(m.group(5))
            mem = float(m.group(6))
            tok_s = float(tok_s_raw.replace("K", "")) * (
                1000 if "K" in tok_s_raw else 1
            )
            tok_s_gpu = float(tok_s_gpu_raw.replace("K", "")) * (
                1000 if "K" in tok_s_gpu_raw else 1
            )
            steps_data.append(
                {
                    "step": step,
    ...
    if not steps_data:
        print("  No metrics parsed from output")
    ...
    result_dict = {
        ...
        "seq": seq,
        "gc": gc,
        "tokens_per_sec": round(avg_tok),
        "tokens_per_sec_gpu": round(avg_tok_gpu),
        "mfu": round(avg_mfu, 1),
        "memory_gb": round(avg_mem, 1),
        "final_loss": round(final_loss, 4),
        "elapsed": round(elapsed, 1),
    }
    print(
        f"  OK | Tok/s: {avg_tok:,.0f} | Tok/s/GPU: {avg_tok_gpu:,.0f} "
...
def main():
    ...
    if ok:
        ...
        for r in ok:
            print(
                f"{r['label']:<30} {r['npus']:>5} {r['tokens_per_sec']:>8,} "
