...
def _build_model(config: LlamaConfig, args, device, rank):
    """Create LlamaForPretraining, wrap with FSDP2."""
    # NOTE: Do NOT use meta device + to_empty for TP+FSDP2.
    # DTensor parameters from to_empty contain uninitialized memory
    # that reset_parameters() cannot properly handle, causing NaN.
    # Instead, create on CPU, init weights, then apply FSDP2.
