...
```mermaid
graph TD
    A[GPU 0 - Data D] -- Broadcast (D) --> B[GPU 1 - Data D]
    A -- Broadcast (D) --> C[GPU 2 - Data D]
    A -- Broadcast (D) --> D[GPU 3 - Data D]
...
如下图所示，主节点 0 将数据分为四份（不同颜色方块代表不同的数据），分别分发给了节点 0 到 3。

```mermaid
graph TD
    A[GPU 0 - D = A B C D] --> B{Scatter}
    B --> C[GPU 0 <br> A]
    B --> D[GPU 1 <br> B]
    B --> E[GPU 2 <br> C]
    B --> F[GPU 3 <br> D]
...
```
**说明:** 根节点（GPU 0）将数据 $D$ **切片**（$A, B, C, D$）后，将不同的分片发送给对应的节点。
...
graph TD
    ...
    style R fill:#ddf,stroke:#333
```
**说明:** 所有节点的数据 $D_i$ 经过**规约运算**（如 $\sum$ 求和）后，最终的**结果 $R$** 被同步到所有节点上.
...
如下图所示，假设执行 SUM 规约：先对 GPU 0-3 上的数据进行规约，得到完整结果 $A(A_0+A_1+A_2+A_3) $ + $ B(B_0+B_1+B_2+B_3) $ + $ C(C_0+C_1+C_2+C_3)$ + $D(D_0+D_1+D_2+D_3)$。然后，将这个完整结果切片（Scatter）分发给所有 GPU 卡，如 GPU 0 获得  $(A_0 + B_0 + C_0 + D_0)$ 分片，GPU 1 获得 $(A_1 + B_1 + C_1 + D_1)$ 分片，GPU 2 获得$ (A_2 + B_2 + C_2 + D_2) $分片，GPU 3 获得 $(A_3 + B_3 + C_3 + D_3)$  分片。

```mermaid
graph TD
    subgraph Reduce Phase
        A[GPU 0 <br> A = A0, A1, A2, A3] --> R(Reduce  <br> Sum = A + B + C + D)
        B[GPU 1 <br> B = B0, B1, B2, B3] --> R
        C[GPU 2 <br> C = C0, C1, C2, C3] --> R
        D[GPU 3 <br> D = D0, D1, D2, D3] --> R
    end
    subgraph Scatter Phase
        R --> E[GPU 0 <br> A0+B0+C0+D0]
        R --> F[GPU 1 <br> A1+B1+C1+D1]
        R --> G[GPU 2 <br> A2+B2+C2+D2]
        R --> H[GPU 3 <br> A3+B3+C3+D3]
    end
    style R fill:#ddf,stroke:#333
```
...
**说明:** 每个节点将自己的数据切片 $D_{ij}$ 发送给**所有**其他节点，同时接收所有节点发来的切片，通常用于矩阵转置。
...
graph LR
    subgraph Data Input
        A[GPU 0 <br> A = A0, A1, A2, A3]
        B[GPU 1 <br> B = B0, B1, B2, B3]
        C[GPU 2 <br> C = C0, C1, C2, C3]
        D[GPU 3 <br> D = D0, D1, D2, D3]
    end
    subgraph Exchange
        A --> T01(Send A0 to GPU 1)
        A --> T02(Send A1 to GPU 2)
        A --> T03(Send A2 to GPU 3)
        A --> T04(Send A3 to GPU 0)
        B --> T10(Send B0 to GPU 0)
        B --> T11(Send B1 to GPU 2)
        B --> T12(Send B2 to GPU 3)
        B --> T13(Send B3 to GPU 1)
        C --> T20(Send C0 to GPU 1)
        C --> T21(Send C1 to GPU 3)
        C --> T22(Send C2 to GPU 0)
        C --> T23(Send C3 to GPU 1)
        D --> T30(Send D0 to GPU 1)
        D --> T31(Send D1 to GPU 2)
        D --> T32(Send D2 to GPU 3)
        D --> T33(Send D3 to GPU 0)
    end
    subgraph Data Output
        E[GPU 0 <br> A0, B0, C0, D0]
        F[GPU 1 <br> A1, B1, C1, D1]
        G[GPU 2 <br> A2, B2, C2, D2]
        H[GPU 3 <br> A3, B3, C3, D3]
    end
```
...
在分布式训练过程中，深度学习训练框架通常**不会直接操作底层通信网络**，而是通过使用专业的**网络通信库（如 NCCL、MPI 等）**来完成数据的集合通信。
