...
def init_model_with_dematerialized_weights(
    ...
    def register_empty_parameter(
        module: nn.Module, name: str, param: nn.Parameter | None
    ) -> None:
    ...
    def register_empty_buffer(
        module: nn.Module, name: str, buffer: torch.Tensor | None
    ) -> None:
...
def init_model_with_materialized_weights(
    model: nn.Module, model_config: Any, save_dir: str | Path
) -> nn.Module:
    ...
    if not has_checkpoint:
        rank_print(
            f"Rank {rank}: No checkpoint in {save_dir}. Materializing with random init."
        )
    ...
    if index_path.exists():
        state_dict = _load_sharded_checkpoint(
            save_dir, index_path, layer_names, initialization_manager
        )
    else:
        state_dict = _load_single_checkpoint(
            safetensors_path, layer_names, initialization_manager
        )
    ...
    if not use_strict and result.unexpected_keys:
        rank_print(
            f"Warning: {len(result.unexpected_keys)} unexpected keys "
...
def _load_sharded_checkpoint(
    ...
    rank_print(
        f"Rank {getattr(pgm, 'global_rank', 0)}: "
...
class InitializationManager:
    ...
    def get_layer_names_in_sft_format(self) -> list[str]:
        ...
        if model_type == "qwen3_moe":
            decoder_components = [
                c for c in decoder_components if not c.startswith("mlp.")
            ]
            ...
            for eid in range(start_expert, end_expert):
                for proj in ("gate_proj", "up_proj", "down_proj"):
                    decoder_components.append(f"mlp.experts.{eid}.{proj}")
        ...
        if isinstance(self.model, PipelineParallel):
            base_names = [
                f"model.layers.{layer_id}" for layer_id in self.model.layer_distribution
            ]
        else:
            base_names = [
    ...
    def adjust_tensor_size(self, tensor: torch.Tensor, name: str) -> torch.Tensor:
        ...
        # Handle MLP layers
            ...
            if "up_proj.weight" in name or "gate_proj.weight" in name:
                ...
                and tensor.shape[1] != intermediate_size_per_rank
            ):
                start_idx = tp_rank * intermediate_size_per_rank
                end_idx = start_idx + intermediate_size_per_rank
                tensor = tensor[:, start_idx:end_idx]
        ...
        if "mlp.experts." in result:
            ...
            if m:
                ...
                result = result.replace(
                    f"mlp.experts.{global_id}.", f"moe.experts.experts.{local_id}."
                )
...
class CheckpointManager:
    ...
    def _get_checkpoint_path(self, out_dir: str | Path) -> Path:
    ...
    def load_checkpoint(
        self, model: nn.Module, optimizer: torch.optim.Optimizer, out_dir: str | Path
    ) -> tuple[int, int]:
