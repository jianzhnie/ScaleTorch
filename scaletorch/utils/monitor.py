...
class PerformanceMonitor:
    ...
    def __init__(self, config: Any, log_dir: str | None = None):
    ...
    def end_iteration(self, tokens_processed: int | None = None) -> dict[str, Any]:
        ...
        total_time = (
            time.time() - self.start_time if self.start_time else iteration_time
        )

        # Calculate throughput
        tokens_per_second = (
            self.iteration_tokens_processed / iteration_time
            if iteration_time > 0
            else 0
        )
        ...
        cpu_stats = {
            ...
            "cpu_memory_available": psutil.virtual_memory().available / (1024**2),  # MB
        }
        ...
        iteration_stats = {
            ...
            "tokens_processed": self.iteration_tokens_processed,
            "iteration_time": iteration_time,
            "total_time": total_time,
            "tokens_per_second": tokens_per_second,
            "iterations_per_second": 1.0 / iteration_time if iteration_time > 0 else 0,
            "gpu": gpu_stats,
            "cpu": cpu_stats,
            "memory": memory_stats,
    ...
    def get_average_stats(self) -> dict[str, Any]:
        ...
        avg_stats = {
            "average_iteration_time": sum(s["iteration_time"] for s in self.stats)
            / len(self.stats),
            "average_tokens_per_second": sum(s["tokens_per_second"] for s in self.stats)
            / len(self.stats),
            "average_iterations_per_second": sum(
                s["iterations_per_second"] for s in self.stats
            )
            / len(self.stats),
            "total_tokens_processed": sum(s["tokens_processed"] for s in self.stats),
            "total_time": self.stats[-1]["total_time"],
            "total_iterations": len(self.stats),
        }
        ...
        if is_accelerator_available() and "gpu" in self.stats[0]:
            avg_stats["gpu"] = {
                "avg_gpu_utilization": sum(
                    s["gpu"].get("gpu_utilization", 0) for s in self.stats
                )
                / len(self.stats),
                "avg_gpu_memory_allocated": sum(
                    s["gpu"].get("gpu_memory_allocated", 0) for s in self.stats
                )
                / len(self.stats),
            }
    ...
    def save_stats(self, filename: str | None = None) -> str | None:
