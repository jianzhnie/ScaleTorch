...
class LrSchedulerArguments:
    ...
    max_lr: float | None = field(
        default=None,
        metadata={"help": "Upper learning rate boundary for OneCycleLR"},
    )
    pct_start: float = field(
        default=0.3,
        metadata={"help": "Percentage of the cycle spent increasing LR for OneCycleLR"},
    )
...
class ScaleTorchArguments(
    ...
    global_batch_size_token: int | None = field(init=False, default=None)
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            and self.context_parallel_size
