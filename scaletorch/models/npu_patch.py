...
if torch_npu is not None:
    ...
    def apply_rotary_pos_emb_flashatt_qwen2_5_vl_npu(
        ...
        q_embed = apply_rotary_emb(
            ...
            sin.unsqueeze(0).unsqueeze(2).float()).type_as(q)
        k_embed = apply_rotary_emb(
            k.float(),
            cos.unsqueeze(0).unsqueeze(2).float(),
            sin.unsqueeze(0).unsqueeze(2).float()).type_as(k)
    ...
    class GmmFunction(torch.autograd.Function):
        ...
        def forward(ctx, x, weight, group_list, split_size):
            ...
            outputs = torch_npu.npu_grouped_matmul([x], [weight],
        ...
        def backward(ctx, grad_outputs):
            ...
            xt = x.permute(1, 0)
            dx = torch_npu.npu_grouped_matmul([grad_outputs], [wt],
                                              group_list=group_list,
                                              group_type=0,
                                              split_item=2)
            ...
            with torch.npu.amp.autocast(enabled=False):
                dw = torch.stack([
                    torch.matmul(xt_list[i], grad_outputs_list[i])
    ...
    def moe_block_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        up_weight_list = [
            e.up_proj.weight.t().to(input_dtype) for e in self.experts
        ]
        gate_weight_list = [
            e.gate_proj.weight.t().to(input_dtype) for e in self.experts
        ]
        down_weight_list = [
            e.down_proj.weight.t().to(input_dtype) for e in self.experts
        ]
    ...
    def _check_and_enable_flash_attn_2(
        ...
        if not cls._supports_flash_attn_2:
            raise ValueError(
                f'{cls.__name__} does not support Flash Attention 2.0 yet.'
                ' Please request to add support where the'
