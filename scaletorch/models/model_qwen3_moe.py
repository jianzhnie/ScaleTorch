...
from scaletorch.models.llama import (FinalProjection, FusedRMSNorm,
                                     LlamaEmbedding, LlamaRMSNorm,
                                     apply_rotary_pos_emb, flash_attention,
                                     get_cos_sin)
from scaletorch.models.model_qwen3 import Qwen3Attention, Qwen3RMSNorm
...
class MoERouter(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        topk_weights, topk_indices = torch.topk(
            logits, self.top_k, dim=-1)  # [num_tokens, top_k]
...
class MoELayer(nn.Module):
    ...
    def __init__(self, config: Any):
        ...
        self.experts = MoEExperts(
            self.experts_per_rank, config.hidden_size,
    ...
    def _forward_ep(self, hidden_flat, topk_weights, topk_indices):
        ...
        recv_tokens, recv_expert_ids, recv_weights, \
            send_splits, recv_splits, reorder_idx = dispatch_tokens(
        ...
        gathered = gather_tokens(
            expert_output, send_splits, recv_splits,
            reorder_idx, num_tokens, self.top_k, hidden)
...
class Qwen3MoEDecoderLayer(nn.Module):
    ...
    def __init__(self, config: Any, layer_idx: int):
        ...
        RMSNorm = LlamaRMSNorm if os.getenv(
            'FLASH_ATTEN', '1') != '1' else FusedRMSNorm
        ...
        cos, sin = get_cos_sin(
            ...
            base=rope_theta, device=torch.device('cpu'))
